...
class ChatBox(object):
    ...
    def add_permission(self, user_to_add, nome_grupo):
        ...
        if user_to_add in self.channel_permissions[nome_grupo]:
            ...
            self.cursor.execute("SELECT servidor_id_grupo FROM tb_grupo WHERE nome_grupo = %s", (nome_grupo, ))
    ...
    def enter_group(self, nome_grupo, usuario1):
        ...
        query = """
                INSERT INTO tb_grupo (servidor_id_grupo, nome_grupo, servidor_id_user)
                VALUES (%s, %s, %s);
                """
        self.cursor.execute(query, (servidor_id_grupo, nome_grupo, servidor_id_user))
        self.conn.commit()
        ...
        return ("Entrou no grupo com sucesso")
    ...
    def create_group(self, nome_grupo, usuario_adm, descricao=None):
        ...
        query = "INSERT INTO tb_servidor_grupo (descricao, grupo_adm_user) VALUES (%s, %s);"
        self.cursor.execute(query, (descricao, grupo_adm_user))
        
        self.cursor.execute("SELECT id_grupo FROM tb_servidor_grupo WHERE grupo_adm_user = %s ORDER BY id_grupo DESC LIMIT 1", (grupo_adm_user,))
        servidor_id_grupo = self.cursor.fetchone()
        servidor_id_grupo = servidor_id_grupo[0]
        
        sql = "INSERT INTO tb_grupo (servidor_id_grupo, nome_grupo, servidor_id_user) VALUES (%s, %s, %s)"
        self.cursor.execute(sql, (servidor_id_grupo, nome_grupo, grupo_adm_user))
        ...
        return print("GRUPO CRIADO COM SUCESSO!!!")

    def send_message_group(self, nome_grupo, usuario1, mensagem): # criar logs
        

        for (n, c) in self.channels[nome_grupo][:]:  # use a copy of the list
            try:
                c.message(usuario1, mensagem)  # oneway call
    ...
    def create_chat_privado(self, usuario1, usuario2):
        ...
        self.cursor.execute("SELECT id_user FROM tb_usuario WHERE nome = %s", (usuario1, ))
        usu1 = self.cursor.fetchall()
        usu1 = usu1[0][0]

        
        self.cursor.execute("SELECT id_user FROM tb_usuario WHERE nome = %s", (usuario2, ))
        usu2 = self.cursor.fetchall()
        usu2 = usu2[0][0]

        

        query = "INSERT INTO tb_servidor_privado (servidor_id_user1, servidor_id_user2) VALUES (%s, %s)"
        self.cursor.execute(query, (usu1, usu2))

        self.conn.commit()

        self.dm_channels[(usuario1, usuario2)] = (self.instance_user.get(usuario1), self.instance_user.get(usuario2))
        print(self.dm_channels)

        self.instance_user.get(usuario1).message()
        return print("DM CRIADO COM SUCESSO!!!")
